...
const ROLE_GRANTS: Record<string, { desc: string; perms: string[] }> = {
  ...
  "QHSE Manager": {
    ...
    perms: [
      ...
      "flaginsp:read", "flaginsp:create", "flaginsp:update", "flaginsp:close", "flaginsp:delete",
      ...
      "opex:read", "opex:manage",
      "budget:read", "budget:manage", "calendar:read", "calendar:manage",
  ...
  "Marine Superintendent": {
    ...
    perms: [
      ...
      "flaginsp:read", "flaginsp:create", "flaginsp:update",
  ...
  "Ship Officer": {
    ...
    perms: [
      ...
      "iaudit:read", "eaudit:read", "flaginsp:read", "cinsp:read",
      ...
      // stays with {psc,iaudit,eaudit,flaginsp,cinsp}:update (office-only).
      "capa:respond",
      "exposure:read", "exposure:create", "exposure:update",
...
const USERS: {
  ...
  // (Swan Aquarius has no dedicated login — use Administrator's "View as
  // Vessel" toggle to preview any vessel's side instead.)
...
async function main() {
  ...
  for (let i = 0; i < SCHEDULE_ITEMS.length; i++) {
    ...
    const item = await prisma.scheduleItem.upsert({
      where: { companyId_kind_flag_itemNo: { companyId: company.id, kind: s.kind, flag: "", itemNo: s.itemNo } },
      update: {
        category: s.category,
        name: s.name,
  ...
  if (!(await prisma.riskAssessmentDocument.findFirst({ where: { companyId: company.id, refNo: "RA-2026-0001" } }))) {
    ...
    if (firstVessel) {
      // Demonstrates the per-job reassessment feature: the vessel's own
      // actual Severity/Likelihood for this specific job, which may differ
      // from the master template's rating (here, likelihood assessed lower
      // than the template's default because the area was already clear and
      // well-ventilated for this particular job).
      const combustiblesRow = raDoc.revisions[0]!.hazardRows.find((r) => r.rowNo === 1);
      const fireExplosionRow = raDoc.revisions[0]!.hazardRows.find((r) => r.rowNo === 3);
      await prisma.riskAssessmentExecution.create({
        data: {
          ...
          hazardSelections: {
            create: [
              ...(combustiblesRow
                ? [{ companyId: company.id, hazardRowId: combustiblesRow.id, severity: 4, likelihood: 1, resLikelihood: 1, createdBy: adminId || null }]
                : []),
              ...(fireExplosionRow
                ? [{ companyId: company.id, hazardRowId: fireExplosionRow.id, severity: 4, likelihood: 1, resLikelihood: 1, createdBy: adminId || null }]
                : []),
            ],
          },
        },
