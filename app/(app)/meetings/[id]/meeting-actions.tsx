  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        {isOpen ? (
          canClose && (
            <Button onClick={() => run(closeMeetingAction)} disabled={pending}>
              <CheckCircle2 className="h-4 w-4" /> Close Meeting
            </Button>
          )
        ) : (
        ...
        {canDelete && (
          <Button
            variant="outline"
            onClick={() => {
              if (confirm("Delete this meeting record?")) run(deleteMeetingAction);
            }}
            disabled={pending}
          >
