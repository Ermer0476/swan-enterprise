...
import { Plus, Flag, BarChart3 } from "lucide-react";
import { requirePermission, can } from "@/lib/rbac";
import { listFlagInspections } from "@/features/flag-inspections/queries";
import { INSPECTION_STATUSES } from "@/features/flag-inspections/schema";
...
export default async function FlagInspectionsPage({
  searchParams,
}: {
  ...
  const { rows, total, page, totalPages } = await listFlagInspections(
    user.companyId,
    {
  ...
  const canCreate = can(user, "flaginsp:create");

  return (
    <>
      <PageHeader
        title="Flag Inspections"
        description="Flag State inspections and their finding close-out."
        actions={
          <div className="flex items-center gap-2">
            <Link href="/flag-inspections/kpi">
              <Button variant="warning">
                <BarChart3 className="h-4 w-4" /> Flag KPIs
              </Button>
            </Link>
            {canCreate && (
              <Link href="/flag-inspections/new">
                <Button>
      ...
      {rows.length === 0 ? (
        <Card className="flex flex-col items-center justify-center gap-2 py-16 text-center">
          ...
          <p className="text-sm font-medium">No flag inspections recorded</p>
          <p className="max-w-sm text-sm text-muted-foreground">
            {canCreate ? "Record a flag inspection and its findings." : "No inspections match your filters."}
          </p>
        </Card>
      ) : (
        <Card className="overflow-hidden">
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                    <td className="px-4 py-2.5 font-mono text-xs">
                      <Link href={`/flag-inspections/${r.id}`} className="text-accent hover:underline">{r.refNo}</Link>
                    </td>
                    <td className="px-4 py-2.5">
                      <Link href={`/flag-inspections/${r.id}`} className="hover:underline">{r.scope}</Link>
                    </td>
          ...
          <Pager page={page} totalPages={totalPages} total={total} basePath="/flag-inspections" searchParams={sp} />
        </Card>
      )}
    </>
  );
