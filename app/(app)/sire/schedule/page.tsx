...
export default async function SireSchedulePage() {
  ...
  const rows = await listSireSchedule(user.companyId, isShipboard ? (user.vesselId ?? undefined) : undefined);
  ...
  return (
    <>
      ...
      <PageHeader
        title="SIRE Schedule"
        description={`${isShipboard ? "Next-inspection matrix" : "Fleet-wide next-inspection matrix"} — the next SIRE is scheduled ${SIRE_SCHEDULE_MONTHS} months after the last one (reports are valid ${SIRE_VALIDITY_MONTHS} months; the extra month is buffer so validity never lapses).`}
      />
