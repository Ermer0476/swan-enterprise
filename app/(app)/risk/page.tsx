...
import { Plus, ClipboardList, Library, MessagesSquare, ChevronLeft, ChevronRight } from "lucide-react";
import { requirePermission, can } from "@/lib/rbac";
import { listAllExecutions, executionKpis, listVesselOptions, listVesselFeedback } from "@/features/risk/queries";
...
export default async function RiskExecutionsPage({
  ...
  const canReviewControls = can(user, "risk-doc:approve");
  const [kpis, vesselFeedback] = await Promise.all([
    canCreate ? executionKpis(user.companyId) : Promise.resolve(null),
    canReviewControls ? listVesselFeedback(user.companyId) : Promise.resolve([]),
  ]);
  const pendingFeedbackCount = vesselFeedback.filter((f) => f.status === "PENDING").length;
    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap justify-end gap-4">
        {canReviewControls && (
          ...
          >
            <MessagesSquare className="h-4 w-4" /> Vessel Feedback
            {pendingFeedbackCount > 0 && <Badge tone="warning">{pendingFeedbackCount}</Badge>}
          </Link>
        )}
