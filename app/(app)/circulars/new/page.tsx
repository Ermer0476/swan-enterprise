...
import { listVesselOptions, listOfficeUserOptions, getIssuingBodySuggestions } from "@/features/circulars/queries";
...
export default async function NewCircularPage() {
  ...
  const [vessels, officeUsers, issuingBodySuggestions] = await Promise.all([
    listVesselOptions(user.companyId),
    listOfficeUserOptions(user.companyId),
    getIssuingBodySuggestions(user.companyId),
  ]);
  return (
    <div className="mx-auto max-w-7xl">
      ...
      />
      <NewCircularForm vessels={vessels} officeUsers={officeUsers} issuingBodySuggestions={issuingBodySuggestions} />
    </div>
  );
}
