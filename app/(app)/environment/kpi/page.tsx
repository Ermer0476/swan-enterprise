...
import {
  getFleetEnvironmentMonthlyGarbage,
  getFleetEnvironmentMonthlyDischarge,
  getFleetEnvironmentGarbageByDisposalMethod,
 ...
 * months it covers and a human label — mirrors the per-vessel KPI page's
 * resolvePeriod, drives the Total Cargo Loaded tile without a second query. */
...
export default async function EnvironmentFleetKpiPage({
  searchParams,
}: {
  ...
  // This page is a fleet-wide comparison by design — a shipboard session
  // (real Ship Officer or an Administrator's View-as-Vessel) has no "other
  // vessels" to compare against, so send it straight to the per-vessel KPI
  // page instead of rendering full fleet data.
  if (user.department === "SHIPBOARD" && user.vesselId) {
    redirect(`/environment/${user.vesselId}/kpi`);
  }
  ...
  const [monthlyGarbage, monthlyDischarge, garbageByMethod] = await Promise.all([
    getFleetEnvironmentMonthlyGarbage(user.companyId, year),
    getFleetEnvironmentMonthlyDischarge(user.companyId, year),
    getFleetEnvironmentGarbageByDisposalMethod(user.companyId, year),
  ]);
  ...
  // Ballast Water — one vertical bar per month, fleet total.
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <PageHeader title="Fleet Environment KPI Dashboard" description={`Fleet-wide garbage and discharge figures for ${year} — every vessel combined.`} />
        ...
        tabs={[
          {
            ...
            content:
              garbageByMonth.length === 0 ? (
                <p className="text-sm text-muted-foreground">No garbage entries logged fleet-wide for {year}.</p>
              ) : (
                <div className="space-y-6">
                  ...
                  <div className="border-t border-border pt-6">
                    <div className="mb-3 text-sm font-semibold">Disposal Method by Category — {year} Fleet Total</div>
                    <p className="mb-3 text-xs text-muted-foreground">
                      Per category, how much (cu.m.) the whole fleet discharged to sea, incinerated, or discharged ashore for the year.
                    </p>
                    <StackedBarChart
          ...
          },
          {
            ...
            content:
              dischargeMonths.length === 0 ? (
                <p className="text-sm text-muted-foreground">No records logged fleet-wide for {year}.</p>
              ) : (
          ...
          },
          {
            ...
            content:
              singleMetricPoints("refrigerantQuantityKg").length === 0 && singleMetricPoints("refrigerantAdded").length === 0 ? (
                <p className="text-sm text-muted-foreground">No refrigerant figures logged fleet-wide for {year}.</p>
              ) : (
                <div className="space-y-6">
          ...
          },
          {
            ...
            content:
              singleMetricPoints("cargoLoaded").length === 0 ? (
                <p className="text-sm text-muted-foreground">No cargo figures logged fleet-wide for {year}.</p>
              ) : (
                <div className="space-y-4">
                  <Card>
                    <CardContent className="pt-5">
                      ...
                      <div className="text-xs text-muted-foreground">Total Fleet Cargo Loaded — {periodLabel}</div>
                    </CardContent>
                  </Card>
