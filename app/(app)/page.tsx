...
import { VerticalBarChart, paletteColor, type BarDatum } from "@/components/ui/bar-chart";
...
import { flagInspectionAnalytics } from "@/features/external-audits/queries";
...
export default async function DashboardPage({
  ...
  // things the external inspectors catch — office-only, so gated on every
  // module's own read permission rather than shown to shipboard accounts.
  const canSeeInspectionComparison =
    can(user, "sire:read") &&
    can(user, "psc:read") &&
    can(user, "cdi:read") &&
    can(user, "eaudit:read") &&
    can(user, "cinsp:read");
  ...
  const [sireKpi, pscKpi, cdiKpi, flagKpi, cinspKpi] = canSeeInspectionComparison
    ? await Promise.all([
        sireAnalytics(user.companyId, comparisonRange),
        pscAnalytics(user.companyId, pscRange),
        cdiAnalytics(user.companyId, comparisonRange),
        flagInspectionAnalytics(user.companyId, comparisonRange),
        companyInspectionAnalytics(user.companyId, comparisonRange),
      ])
    : [null, null, null, null, null];

  const canSeeSireSchedule = can(user, "sire:read");
  const [sireObsStat, sireObsTargets] = canSeeSireSchedule
    ? await Promise.all([sireAnalytics(user.companyId, {}), getSireKpiTargets(user.companyId)])
    : [null, null];
  ...
  // itself is cheap (2 lightweight fleet-wide queries each), so the small
  // duplication against getFleetVesselHealth's own copy of this data isn't
  // worth the RBAC correctness it would cost to dedupe.
  const sireAlerts = canSeeSireSchedule ? sireScheduleAlerts(await listSireSchedule(user.companyId)) : [];
  const canSeeInternalAuditSchedule = can(user, "iaudit:read");
  const iAuditAlerts = canSeeInternalAuditSchedule
    ? internalAuditScheduleAlerts(await listInternalAuditSchedule(user.companyId))
    : [];
  ...
  // Fleet-wide Safety Performance tiles — gated on holding read access to
  // every module feeding them, same convention as canSeeInspectionComparison
  // above (a permission check, not a department check — shipboard accounts
  // that hold these read perms see the same fleet-wide picture office does,
  // matching how the Inspection Effectiveness chart already behaves).
  ...
  const [incidentKpis, exposureKpi, exposureTargets, capaRate, nearMissCounts] = canSeeSafetyPerformance
    ? await Promise.all([
        ...
        getRollingKpiDashboard(user.companyId, {}),
        getExposureKpiTargets(user.companyId),
        getFleetCapaClosureRate(user.companyId),
        getNearMissReportingCounts(user.companyId),
      ])
    : [null, null, null, null, null];
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-7">
            ...
            <Card>
              <CardContent className="pt-4">
                ...
                <div className="mt-0.5 text-xs text-muted-foreground">CAPA Closure Rate (Fleet-wide)</div>
                {capaRate.total > 0 && (
