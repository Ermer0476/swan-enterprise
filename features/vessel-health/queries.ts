...
// same 8 types as ALL_CAPA_ENTITY_TYPES in features/capa/queries.ts. Every
...
const CAPA_ENTITY_TYPE_LABELS: Record<string, string> = {
  ...
  FlagInspectionFinding: "Flag Inspection",
...
async function computeFleetIncidentCapaSignals(companyId: string): Promise<{
  ...
  const [incidents, nearMisses, ncrs, pscInspections, internalAudits, externalAudits, flagInspections, companyInspections] =
    await Promise.all([
      ...
      prisma.flagInspection.findMany({ where: { companyId, deletedAt: null }, select: { id: true, vesselId: true } }),
  ...
  const flagInspectionIds = flagInspections.map((a) => a.id);
  const companyInspectionIds = companyInspections.map((c) => c.id);

  const [pscDeficiencies, internalAuditFindings, externalAuditFindings, flagInspectionFindings, companyInspectionObservations] =
    await Promise.all([
      ...
      externalAuditIds.length
        ...
        : Promise.resolve([]),
      flagInspectionIds.length
        ? prisma.flagInspectionFinding.findMany({ where: { companyId, auditId: { in: flagInspectionIds }, deletedAt: null }, select: { id: true, auditId: true } })
        : Promise.resolve([]),
  ...
  const flagInspectionVessel = new Map(flagInspections.map((a) => [a.id, a.vesselId]));
  const companyInspectionVessel = new Map(companyInspections.map((c) => [c.id, c.vesselId]));

  // entityId -> vesselId, across all 8 CAPA-adopting entity types.
  ...
  for (const r of flagInspectionFindings) entityVessel.set(`FlagInspectionFinding:${r.id}`, flagInspectionVessel.get(r.auditId) ?? null);
