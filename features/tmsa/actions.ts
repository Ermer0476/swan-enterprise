...
export async function createFindingAction(_prev: NewFindingState, formData: FormData): Promise<NewFindingState> {
  ...
  // Compute the sequence + code and create inside a retry: two concurrent adds
  // would derive the same seq and collide on @@unique([companyId, code]); on a
  // P2002 we recompute the seq from the now-committed row and try again.
  const { finding, code } = await withUniqueRetry(async () => {
