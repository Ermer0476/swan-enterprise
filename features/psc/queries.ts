...
export async function pscAnalytics(companyId: string, range: { from: Date; to: Date }): Promise<PscKpiData> {
  const inspections = await prisma.pscInspection.findMany({
    where: { companyId, deletedAt: null, inspectionDate: { gte: range.from, lte: range.to } },
