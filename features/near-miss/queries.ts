...
export async function nearMissAnalytics(companyId: string, range: { from?: Date; to?: Date }): Promise<NearMissKpiData> {
...
export async function getNearMissReportingCounts(companyId: string, asOf: Date = new Date()): Promise<{ last30: number; previous30: number }> {
  ...
  const [last30, previous30] = await Promise.all([
    prisma.nearMiss.count({
      where: { companyId, deletedAt: null, status: { not: "DRAFT" }, occurredAt: { gte: start30, lte: asOf } },
    }),
    prisma.nearMiss.count({
      where: { companyId, deletedAt: null, status: { not: "DRAFT" }, occurredAt: { gte: start60, lt: start30 } },
    }),
  ]);
