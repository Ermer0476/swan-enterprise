 ...
 * This builds that schedule for every active vessel (or just one, when
 * `vesselId` is passed — a shipboard session should only ever see its own
 * ship's schedule/alert count, not the whole fleet's): its last SIRE
 * inspection date, when the next one is scheduled (+5mo), when the current
 * report's validity hard-expires (+6mo), and an urgency bucket — sorted so
 * the vessel needing attention soonest is always on top.
 */
export async function listSireSchedule(companyId: string, vesselId?: string): Promise<SireScheduleRow[]> {
...
export async function sireAnalytics(companyId: string, range: { from?: Date; to?: Date }, vesselId?: string) {
