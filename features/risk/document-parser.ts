...
export function parseRaHazardTable(
  ...
  let cursor = respIdx + 1;
  // Skip any note row(s) between the header and the first real data row.
  while (cursor < cells.length && !ROW_LABEL_PATTERN.test(cells[cursor]!)) cursor++;
