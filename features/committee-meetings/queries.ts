...
export async function getCommitteeMeeting(user: SessionUser, id: string) {
  ...
  // CommitteeMeetingTypeRemarks only stores a raw updatedBy id (the generic
  // audit-field convention), so resolving it to a name needs a manual
  // lookup rather than a Prisma include/relation.
  const typeRemarksAuthorIds = meeting.typeRemarks.map((r) => r.updatedBy).filter((v): v is string => !!v);
  const authors = typeRemarksAuthorIds.length
    ? await prisma.user.findMany({ where: { id: { in: typeRemarksAuthorIds } }, select: { id: true, fullName: true, rank: true } })
    : [];
  ...
  return {
    ...
    typeRemarks: meeting.typeRemarks.map((r) => ({
      ...r,
      updatedByUser: r.updatedBy ? (authorById.get(r.updatedBy) ?? null) : null,
    })),
  };
}
