...
export async function postOpeningStockTakeAction(formData: FormData): Promise<ActionResult> {
  ...
  const stockTake = await prisma.$transaction(async (tx) => {
    // The upsert is keyed on the unique `vesselId`, so two concurrent posts
    // serialize on that row (INSERT … ON CONFLICT takes a row lock). To stay
    // idempotent under that race — and on any accidental re-post — clear this
    // stock take's existing OPENING events before recreating them, so the
    // inventory can never be doubled.
    ...
    await tx.inventoryEvent.deleteMany({ where: { openingStockTakeId: st.id } });
