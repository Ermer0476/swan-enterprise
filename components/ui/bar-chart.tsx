...
export function StackedBarChart({ data, legend }: { data: StackedDatum[]; legend: LegendEntry[] }) {
  // Segment values are already rounded individually, but summing several
  // rounded floats can still land on a binary-float artifact (e.g.
  // 1.31 + 4.49 = 5.800000000000001) — round the total once more for display.
  const totals = data.map((d) => Math.round(d.segments.reduce((s, x) => s + x.value, 0) * 100) / 100);
...
export function VerticalGroupedBarChart({ data, legend }: { data: StackedDatum[]; legend: LegendEntry[] }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-end gap-1 overflow-x-auto pl-4 pb-14">
        {data.map((d) => (
          <div key={d.label} className="flex flex-1 min-w-[3.5rem] flex-col items-center">
            <div className="flex w-full items-end justify-center gap-0.5" style={{ height: TRACK_HEIGHT }}>
              {d.segments.map((s) => {
                const barHeight = Math.max((s.value / max) * BAR_MAX, s.value > 0 ? 2 : 0);
                return (
                  <div key={s.key} className="relative flex h-full w-6 shrink-0 items-end justify-center">
                    <span
